...
- New dashboard-style home screen with live Accessibility service status and quick controls (Start/Pause/Stop/Recorder).
- Improved floating controller with draggable/snap-to-edge behavior, expand/collapse gesture, resume action, recorder toggle, and long-press close.
- Command dispatch now uses an in-app command bus (broadcast actions) for more reliable control signaling to the accessibility runtime.
